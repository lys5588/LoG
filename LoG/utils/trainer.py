...
class Trainer(nn.Module):
    ...
    def check_val(self):
        if "val" not in self.cfg:
            ...
            dataset = load_object(
                self.cfg.val.dataset.module, self.cfg.val.dataset.args
            )
            ...
            if "render" in self.cfg.val:
                self.render_val = load_object(
                    self.cfg.val.render.module, self.cfg.val.render.args
                )
            else:
    ...
    def check_overlook(self):
        if "overlook" not in self.cfg:
            ...
            dataset = load_object(
                self.cfg.overlook.dataset.module, self.cfg.overlook.dataset.args
            )
        ...
        if "overlook_oneframe" not in self.cfg:
            ...
            dataset = load_object(
                self.cfg.overlook_oneframe.dataset.module,
                self.cfg.overlook_oneframe.dataset.args,
            )
    ...
    def init(self, dataset):
        ...
        if "init" in self.cfg.train:
            ...
            for iteration, data in enumerate(
                tqdm(valloader, desc="initialize the model")
            ):
                # timer the process
                self.model.clear()
                batch = prepare_batch(data, self.device)
        ...
        for iteration, data in enumerate(tqdm(quickview, desc="quick view")):
            ...
            cv2.imwrite(
                join(
                    self.exp,
                    "init",
                    f'model_{iteration}_{os.path.basename(batch["imgname"][0])}.jpg',
                ),
                vis,
            )
            if iteration > 10:
    ...
    def log_radius(self, output):
        ...
        self.recorder.writer.add_histogram(
            "train/radius3d", radii, self.global_iterations, bins=100
        )
        # log grad
        ...
        self.recorder.writer.add_histogram(
            "train/grad_norm", grad_norm, self.global_iterations, bins=100
        )

    def log_opacity(self, output):
        opacity = output["opacity"][0][:, 0].detach().cpu().numpy()
        self.recorder.writer.add_histogram(
            "train/opacity", opacity, self.global_iterations, bins=100
        )
    ...
    def log_point_cloud(self, output):
        ...
        outname = os.path.join(
            self.exp, "pointcloud", "{:06d}.ply".format(self.global_iterations)
        )
    ...
    def log_gpu(self):
        ...
        self.recorder.log(
            self.global_iterations, "train/num_points", self.model.num_points
        )
    ...
    def log_grad(self, data):
        ...
        vis_grad_max = 5e-3
        grad_acc = torch.clamp(render[0], 0.0, vis_grad_max) / vis_grad_max
        vis_grad = self.render.marigold_depth_vis(grad_acc)
        cv2.putText(
            vis_grad,
            f"{render[0].min().item():.4f}->{render[0].max().item():.4f}",
            (10, 30),
        ...
        outname = os.path.join(
            self.exp, "grad", "{:06d}.jpg".format(self.global_iterations)
        )
        imwrite(outname, vis_grad)

    @torch.no_grad()
    def log_in_training(
        self, batch_idx, batch_total, data, loss, output, vis_residual=False
    ):
        global_time = time.time() - self.global_start_time
        self.recorder.log(self.global_iterations, "train/time", global_time)
        current_time = time.time() - self.start_time
        print(
            f"[{self.global_iterations:6d}: {batch_idx:6d}/{batch_total:6d}] {current_time:4.1f}s loss: {loss:.4f} model {self.model}"
        )
        ...
        outname = os.path.join(
            self.exp, "vis", "{:06d}.jpg".format(self.global_iterations)
        )
        imwrite(outname, vis)
        if vis_residual:
            residual = torch.norm(output["render"][0] - output["gt"][0], dim=0)
            residual = torch.clamp(residual, min=0.0, max=1.0)
            vis_residual = self.render.marigold_depth_vis(residual)
            outname = os.path.join(
                self.exp, "residual", "{:06d}.jpg".format(self.global_iterations)
            )
        ...
        if "acc_map" in output.keys():
            ...
            outname = os.path.join(
                self.exp, "acc", "{:06d}.jpg".format(self.global_iterations)
            )
    ...
    def make_validation(self, iteration, visualize=False):
        ...
        for _data in tqdm(self.val, desc=f"val {iteration}"):
            ...
            model.clear()
            output = self.render_val.vis(
                batch,
                self.model,
                background=torch.ones_like(self.render_val.background),
            )
            ...
            if getattr(model, "view_correction", None) is not None:
                ...
                view_correct = (gt_left * pred_left).sum(dim=-1).sum(dim=-1) / (
                    pred_left**2
                ).sum(dim=-1).sum(dim=-1)
                pred = torch.clamp(pred * view_correct[:, None, None], 0.0, 1.0)
            ...
            if self.lpips is not None:
                ret_lpips = self.lpips(
                    pred[None], gt[None], retPerLayer=False, normalize=True
                )
            ...
            if (iteration + 1) % 1000 == 0 or visualize:
                ...
                outname = join(
                    logdir,
                    f'{batch["index"][0]:06d}_{os.path.basename(metric["imgname"][-1])}.jpg',
                )
    ...
    def make_overlook(self, mode="rgb", iteration=-1):
        ...
        for _iter, _data in enumerate(self.overlook):
            ...
            if mode == "rgb":
                ...
                vis = self.render.acc_to_bgr(output["render"][0][0])
            cv2.putText(
                vis, f"{text}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2
            )
            del output
            outname = os.path.join(
                self.exp,
                "overlook",
                "{}_{:06d}_{:02d}.jpg".format(mode, iteration, _iter),
            )
    ...
    def make_overlook_oneframe(self, iteration=-1):
        ...
        vis = self.render.tensor_to_bgr(output["render"][0])
        cv2.putText(
            vis, f"{iteration}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2
        )
        del output
        outname = os.path.join(
            self.exp, "overlook_oneframe", "rgb", "{:06d}.jpg".format(iteration)
        )
        os.makedirs(os.path.dirname(outname), exist_ok=True)
        cv2.imwrite(outname, vis)
        self.model.train()
    ...
    def save_ckpt(self, ckptname):
        ...
        torch.save(
            {"state_dict": state_dict, "global_iterations": self.global_iterations},
            ckptname,
        )
    ...
    def check_iteration(self, stage_name, iteration, cfg_iteration):
        ...
        if isinstance(cfg_iteration, dict):
            ...
            if (
                iteration > iters[0]
                and iteration < iters[1]
                and iteration % iters[2] == 0
            ):
                return True
        return False
